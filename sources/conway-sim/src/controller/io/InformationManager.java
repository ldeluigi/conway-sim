...
public final class InformationManager {
    ...
    private static final String FILENAME = ".conway";
    private static File f = new File(InformationManager.FILENAME);
    ...
    public static void saveProgress(final int level) {
        ...
        try (ObjectOutputStream oStream = new ObjectOutputStream(new FileOutputStream(InformationManager.f))) {
            ...
            Logger.logTime("Couldn't write on the specified file" + InformationManager.FILENAME);
    ...
    private static List<Integer> loadList() {
        ...
        if (InformationManager.f.exists()) {
            try (ObjectInputStream oStream2 = new ObjectInputStream(new FileInputStream(InformationManager.f))) {
                ...
                Logger.logTime("Couldn't read from the specified file " + InformationManager.FILENAME);
    ...
    private static void createfile() {
        if (!InformationManager.f.exists()) {
            try {
                ...
                Logger.logTime("Couldn't create the specified file" + InformationManager.FILENAME);
                Logger.logThrowable(e);
            }
        }
    }
