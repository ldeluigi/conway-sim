...
public class ExtensionGridEditorImpl extends GridEditorImpl implements ExtensionGridEditor {
    ...
    public void selectMode(final boolean flag) {
        if (!selectMode && flag) {
            oldPlacingState = isEnabled();
            super.setEnabled(false);
            selectMode = true;
        } else if (selectMode && cutReady && flag) {
        	super.setEnabled(oldPlacingState);
            selectMode = false;
        }
    }
    ...
    private void select(final int row, final int col) { //when click
        if (lastCol != -1 && lastRow != -1) {
            ...
            cutReady = false;
            this.applyChanges();
            this.getCurrentStatus().set(row, col, this.getCurrentStatus().get(row, col));
            this.getGameGrid().displaySingleCell(row, col, SELECT.apply(this.getCurrentStatus().get(row, col)));
            if (lastRow > this.getCurrentStatus().getWidth() || lastCol > this.getCurrentStatus().getHeight()) {
                lastRow = -1;
                lastCol = -1;
            }
            lastCol = col;
