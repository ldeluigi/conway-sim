...
/**
 * This class implements the save of a {@link String} in a file .rle, with in
 * addition the name, the author and an optional comment.
 */
public class JInternalFrameSave extends JInternalFrame {
    ...
    /**
     * 
     * @param stringRLEformat
     *            is a format RLE string that contains the two dimensions, the
     *            version and the matrix in standard RLE string
     */
    public JInternalFrameSave(final String stringRLEformat) {
        ...
        this.bSave = SandboxTools.newJButton(ResourceLoader.loadString("save.button.save"),
                ResourceLoader.loadString("save.tooltip.save"));
        this.bSave.setEnabled(false);
        final JButton bExit = SandboxTools.newJButton(ResourceLoader.loadString("save.button.exit"),
                ResourceLoader.loadString("save.tooltip.exit"));
        south.add(this.bSave);
        south.add(bExit);
        ...
        gridPanel.add(this.textName);
        gridPanel.add(author);
        gridPanel.add(this.textAuthor);
        gridPanel.add(comment);
        gridPanel.add(this.textComment);
        general.add(gridPanel, BorderLayout.CENTER);
        this.add(general);
        this.pack();
        this.setVisible(true);
        this.bSave.addActionListener(e -> {
            final String preText = "#N " + this.textName.getText() + System.lineSeparator() + "#O "
                    + this.textAuthor.getText() + System.lineSeparator()
                    + (this.textComment.getText().length() > 0
                            ? ("#C " + this.textComment.getText() + System.lineSeparator())
                            : "");
    ...
    private void enableSave() {
        if (this.textName.getText().length() > 0 && this.textAuthor.getText().length() > 0) {
